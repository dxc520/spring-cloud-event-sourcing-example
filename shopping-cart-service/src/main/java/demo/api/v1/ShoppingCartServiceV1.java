        ...
        if (currentCart != null) {
            ...
            if (inventory != null) {
                ...
                if (checkAvailableInventory(checkoutResult, currentCart, inventoryItems)) {
                    ...
                    try{
                    	System.out.println("use squid create order ...begin ...");
                    	orderResponse=orderServiceClient.send(createOrderRequest);
                    }catch(Exception e){
                    		System.out.println("Oh my God ###################--------------");
                    	e.printStackTrace();
                    }
//                    Order orderResponse = orderServiceClient.send(createOrderRequest);
                    System.out.println("use squid create order . oerderResonse="+orderResponse);
                   
                   
                    if(orderResponse==null){
                    		 System.out.println("use squid create order . oerderResonse is null .then creat by origin ");
                    		   orderResponse = oAuth2RestTemplate.postForObject("http://order-service/v1/orders",
