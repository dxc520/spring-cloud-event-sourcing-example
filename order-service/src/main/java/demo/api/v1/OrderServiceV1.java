...
public class OrderServiceV1 {
	...
	@Autowired
	public OrderServiceV1(OrderRepository orderRepository, OrderEventRepository orderEventRepository,
	 ...
	 * @return
	 */
	public Order createOrder(List<LineItem> lineItems, boolean flag) {
		Account[] accounts = oAuth2RestTemplate.getForObject("http://account-service/v1/accounts", Account[].class);

		Account defaultAccount = Arrays.asList(accounts).stream().filter(Account::getDefaultAccount).findFirst()
				.orElse(null);
		...
		Order newOrder = new Order(defaultAccount.getAccountNumber(),
				defaultAccount.getAddresses().stream()
						.filter(address -> address.getAddressType() == AddressType.SHIPPING).findFirst()
	...
	public Order createOrder(List<LineItem> lineItems) {
		System.out.println("*******dxc****create virtual order ...begin ");

		Address address_vo = new Address("3495 Deer Creek Road", "", "CA", "United States", "Palo Alto", 94304);
		address_vo.setAddressType(AddressType.SHIPPING);

		Account defaultAccount = new Account("user", "12345");

		Set<Address> set = new HashSet<Address>();
		set.add(address_vo);
		defaultAccount.setAddresses(set);
		/**
		 * Account[] accounts =
		 * oAuth2RestTemplate.getForObject("http://account-service/v1/accounts",
		 * Account[].class);
		 * 
		 * Account defaultAccount = Arrays.asList(accounts).stream()
		 * .filter(Account::getDefaultAccount) .findFirst().orElse(null);
		 **/
		if (defaultAccount == null) {
			return null;
		}
	...
	public Boolean addOrderEvent(OrderEvent orderEvent, Boolean validate) throws Exception {
		...
		if (validate) {
			// Validate the account number of the event's order belongs to the
			// user
...
System.out.println("get order .OrderEvents="+orderEvents);
	...
	public List<Order> getOrdersForAccount(String accountNumber) throws Exception {
		...
		return orders.stream().map(order -> getOrder(order.getOrderId(), true)).filter(order -> order != null)
	...
	public boolean validateAccountNumber(String accountNumber) throws Exception {
		...
		if (accounts != null && !Arrays.asList(accounts).stream()
				.anyMatch(acct -> Objects.equals(acct.getAccountNumber(), accountNumber))) {
